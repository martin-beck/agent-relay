...
namespace. The runner compares the namespace inode with its parent, requires immutable engine,
model, and CLI digests, and invokes a pre-staged driver as a JSON argument array without a shell.
...
repository variables supply the driver argument array and staged digests, so normal PR jobs neither
