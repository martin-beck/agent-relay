...
SECRET = re.compile(r"(?:sk-[A-Za-z0-9_-]{12,}|-----BEGIN [A-Z ]+-----|(?i:bearer)\s+[A-Za-z0-9._-]{12,})")
...
def sanitize_capture(frames: list[dict[str, Any]], *, capture_enabled: bool, upstream: str, allowlist: set[str]) -> list[dict[str, Any]]:
    ...
    previous = -1
    for frame in frames:
        if frame.get("direction") not in {"request", "response"} or not isinstance(frame.get("body"), dict):
        ...
        sanitized.append({"atMillis": at, "direction": frame["direction"], "bodyDigest": digest, "body": frame["body"]})
    if not sanitized or len(sanitized) > 10_000:
        raise CassetteError("capture frame count is invalid")
    return sanitized

def replay(cassette: dict[str, Any], requests: list[dict[str, Any]], *, allow_network: bool = False) -> list[dict[str, Any]]:
    ...
    for frame, request in zip(expected, requests):
        if not isinstance(request, dict) or _validate_body(request, int(cassette["limits"]["maxBodyBytes"])) != frame["bodyDigest"]:
            raise CassetteError("request does not match cassette")
    return [f for f in frames if f.get("direction") == "response"]

def contract_diff(left: dict[str, Any], right: dict[str, Any], volatile: set[str] = set()) -> list[str]:
