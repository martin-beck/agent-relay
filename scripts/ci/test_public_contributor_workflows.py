...
class PublicContributorWorkflowTest(unittest.TestCase):
    ...
    def test_public_runner_is_single_job_and_disposable(self) -> None:
        ...
        self.assertIn("Cannot delete a runner that is currently running a job", supervisor)
        self.assertIn("HTTP 422", supervisor)
        self.assertIn("cleanup_status == 75", supervisor)
        self.assertIn("waiting before retrying cleanup", supervisor)
